...
var instructionSections = []instructionSection{
	...
	},
	{
		...
		text:    "Job state \"broken\" means the pipeline configuration decided, when the job was created, that it would not run: an `if` condition or `branches` filter did not match, `parallelism` was 0, or the step was marked `skip`. Broken is never caused by another job failing, never fails a build, and is normal in passing builds (e.g. main-only deploy steps on a feature branch). Jobs stopped by a failed dependency are `waiting_failed`, `blocked_failed` or `unblocked_failed` instead; those appear only in builds that actually failed. None of these four states ran, so they have no logs — do not fetch them. `failed` = ran and exited non-zero; `skipped` = an external factor (e.g. a newer build superseded it). To explain a red build, investigate `failed`, `timed_out` `expired` jobs as well as `running` jobs that have non-zero `promised_exit_status` only.",
	},
	{
		toolset: toolsets.ToolsetBuilds,
