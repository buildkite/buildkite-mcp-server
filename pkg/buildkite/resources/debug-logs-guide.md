...
**Best first step for diagnosing a build failure** — combines build state, failed and broken jobs, promised failures from still-running jobs, bounded log tails, error/warning annotations, and failed Test Engine executions in one call. Its `job_census` counts every job in the build by state (e.g. `{"total": 4, "passed": 2, "failed": 1, "broken": 1, "truncated": false}`) — when `truncated` is `false`, the summary accounts for all jobs and no follow-up `list_jobs` call is needed to confirm nothing else failed.
