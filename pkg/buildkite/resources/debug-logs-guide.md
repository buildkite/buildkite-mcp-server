...
Each job's `log_tail` is a window of up to 50 lines by default. When the agent's failure marker (`🚨 Error: …`) is found near the end of the log, the window ends just after that marker so it shows the output that led to the failure instead of the cleanup-hook output (artifact uploads, `docker compose down`, pre-exit hooks) that follows; the job reports `log_selection: "error_anchor"` plus `log_anchor_rn` and `log_anchor_group` (the section header the failure was printed under). When no marker is found, for example after a timeout or cancellation, the window is the plain tail and `log_selection` is `"tail"`. Either way, the rows omitted before the window are covered by `log_truncated`, and `log_total_rows` minus the last `rn` tells you how many rows follow it. This window is usually enough for an initial diagnosis. Use the lower-level tools only when the summary identifies an area that needs deeper inspection. You can reduce output with `log_tail`, `max_jobs`, `max_annotations`, `max_test_runs`, `max_failed_tests`, and `max_failed_tests_per_run`, or disable optional sections with `include_logs`, `include_annotations`, and `include_failed_tests`. Test Engine work defaults to at most 5 runs and 100 failed executions total.
